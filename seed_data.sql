-- ═══════════════════════════════════════════════════════════════════
-- MakeEazy FlatOS v2 — OFFICIAL SEED DATA (from Excel DummyData v2)
-- 16 flats, 41 tenants (3-per-flat model), 41 agreements
-- Adapted from Tech_Specs/SQL/002_seed_data.sql
-- ═══════════════════════════════════════════════════════════════════

-- STEP 1: Clean old data (order matters for FK constraints)
...
-- STEP 2: Update org with real data
...
BEGIN
  ...
  INSERT INTO tenants (id,org_id,flat_id,full_name,phone,email,employer_name,ctc_monthly,move_in_date,move_out_date,is_primary,status,rent_share,maint_share) VALUES
    ...
    -- Flat 106: Deloitte team (1 replaced)
  ...
  -- ═══ 41 AGREEMENTS (per-licensee, linked to tenants) ═══
  ...
  -- ═══ DEPOSITS (13 initial deposits for occupied flats) ═══
  ...
  -- ═══ RENT DEMANDS (Jan-Mar 2026 for all occupied flats) ═══
  ...
  INSERT INTO complaints (org_id,flat_id,complaint_number,date,category,description,priority,reported_by,status) VALUES
    (v_org,'30000001-0000-0000-0000-000000000000','CMP-001','2026-01-10','plumbing','Bathroom ceiling leak - water dripping','high','Rajesh Kumar Sharma','open'),
    (v_org,'30000003-0000-0000-0000-000000000000','CMP-002','2026-01-18','electrical','AC compressor noise, not cooling','medium','Vikram Reddy','in_progress'),
    (v_org,'30000005-0000-0000-0000-000000000000','CMP-003','2026-02-05','other','Wall crack in living room widening','high','Ananya Krishnamurthy','open'),
    ...
    (v_org,'30000013-0000-0000-0000-000000000000','CMP-005','2026-02-12','electrical','Frequent voltage drops evenings','high','Pooja Sharma','open');
  ...
  INSERT INTO documents (org_id,flat_id,tenant_id,doc_type,file_name,file_url,expiry_date,is_verified) VALUES
    ...
    -- KYC: Aadhaar for primary tenants
  ...
  RAISE NOTICE '✅ COMPLETE! 16 flats, 41 tenants, 41 agreements (with tenant_id), 13 deposits, 20 demands, 6 vendors, 13 expenses, 5 complaints, 8 compliance, 22 documents (5 agreements + 13 KYC + 4 building)';
END $$;
